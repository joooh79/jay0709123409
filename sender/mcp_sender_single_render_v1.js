...
function getCurrentStateBaseUrl() {
  return CURRENT_STATE_MCP_BASE_URL;
}

function buildCurrentStateHeaders() {
  return {
...
function httpGetJson(url, timeoutMs = 10000) {
  return new Promise((resolve, reject) => {
    ...
    const req = client.request(
      ...
      {
        method: 'GET',
        headers: buildCurrentStateHeaders()
      },
      (res) => {
        let raw = '';
...
function buildCurrentStateFetchUrl(fetchId) {
  const base = getCurrentStateBaseUrl();
  if (!base) {
    throw new Error('CURRENT_STATE_MCP_BASE_URL is not configured');
  }

  return `${base.replace(/\/+$/, '')}/fetch?id=${encodeURIComponent(fetchId)}`;
}

async function fetchVisitByDate(patientId, visitDate) {
  return httpGetJson(buildCurrentStateFetchUrl(`visit_by_date:${patientId}:${visitDate}`));
}

async function fetchVisitChildren(visitRecordId) {
  return httpGetJson(buildCurrentStateFetchUrl(`visit_children:${visitRecordId}`));
}

async function fetchRecord(tableAlias, recordId) {
  return httpGetJson(buildCurrentStateFetchUrl(`record:${tableAlias}:${recordId}`));
}
...
async function buildExistingVisitUpdateCurrentState(transformedPayload) {
  ...
  if (!visitResp.ok) {
    throw new Error(`Target visit fetch failed with status ${visitResp.status}`);
  }

  const visitRecordId = getVisitRecordIdFromBody(visitResp.body);
  if (!visitRecordId) {
    throw new Error('Target visit not found for existing_visit_update');
  }

  const childrenResp = await fetchVisitChildren(visitRecordId);
  if (!childrenResp.ok) {
    throw new Error(`Visit children fetch failed with status ${childrenResp.status}`);
  }

  const preOpRecordsByTooth = {};
  ...
  for (const record of findings) {
    ...
    if (!recordResp.ok) {
      throw new Error(`PRE record fetch failed for ${childRecordId}`);
    }

    preOpRecordsByTooth[toothNumber] = {
  ...
  return {
    ...
    visitChildrenBody: childrenResp.body,
    preOpRecordsByTooth
  };
}
...
function buildPhase1FullPreview(transformedPayload, currentState, headerTouched, findingsTouched, decision) {
  ...
  if (headerTouched.patients.birth_year) {
    headerChanges.push({
      ...
      field: 'birth_year',
      before: '',
      incoming: transformedPayload?.patients?.birth_year || '',
      policy: 'replace',
      after: transformedPayload?.patients?.birth_year || '',
      no_op: false
    });
  }
  ...
  if (headerTouched.patients.gender) {
    headerChanges.push({
      ...
      field: 'gender',
      before: '',
      incoming: transformedPayload?.patients?.gender || '',
      policy: 'replace',
      after: transformedPayload?.patients?.gender || '',
      no_op: false
    });
  }
...
async function buildPhase1TransformEnvelope(payload, transformResult, phase1DecisionRaw) {
  ...
  const stage1Preview = buildPhase1MultiplePreview(transformedPayload, currentState);

  if ((stage1Preview.items || []).length > 0 && phase1DecisionRaw === undefined) {
    return {
      ok: true,
  ...
  return {
    ...
    interaction: {
      mode: 'ask_user',
      ui_kind: 'phase1_full_preview_confirmation',
      user_message: '기존 방문 업데이트 full preview입니다. 이 내용대로 Make/Airtable에 전송하시겠습니까?',
...
async function runSendTool(args) {
  ...
  if (isPhase1ApplicableExistingVisitUpdate(args.payload)) {
    ...
    if (envelope?.phase1?.stage !== 2 || !envelope.transformed_payload) {
      throw new Error(
        'Phase 1 existing_visit_update send requires a valid phase1_decision and completed stage 2 preview'
      );
    }
...
function runHealthTool() {
  return {
    ...
    version: '1.1.0-phase1-existing-visit-update',
