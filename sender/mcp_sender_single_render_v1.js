...
function buildNoOpDisplayRules() {
  return {
    ...
    assistant_mandatory_rules: [
      'If no_op=true, display exactly "변경 없음".',
      'If no_op=false, display exactly "변경 예정".',
      ...
      'Show before/incoming/after together with the exact no_op label when available.'
    ]
  };
}
...
function buildPhase1FullPreview(transformedPayload, currentState, headerTouched, findingsTouched, decision) {
  ...
  const currentStateReliable = !!currentState?.ready;
...
function runHealthTool() {
  return {
    ...
    version: '1.1.14-phase1-existing-visit-update-hotfix14',
