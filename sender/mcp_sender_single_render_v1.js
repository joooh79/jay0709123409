...
async function buildNewPatientPatientFoundTransformEnvelope(payload, transformResult, rawDecision) {
  ...
  if (!(patientResp && patientResp.ok)) {
    return {
      ...
      error: {
        ...
        message: '신규 환자 claim 검증을 위한 current-state patient lookup이 불가능하여 전송을 차단합니다.'
      },
      interaction: {
        mode: 'inform',
        ui_kind: 'info',
        user_message: '현재 환자 존재 여부를 확인할 수 없어 자동 진행을 멈췄습니다.',
        assistant_question: '',
        required_user_input: null,
        do_not_ask: []
      },
      execution_contract: {
        ...
        mode: 'stop',
        must_show_message: true,
        user_visible_message: '현재 환자 존재 여부를 확인할 수 없어 자동 진행을 멈췄습니다.',
        must_ask_user: false,
        user_question: '',
        accepted_input_type: null,
        allowed_actions: ['show_stop_message'],
  ...
  if (!decision) {
    return {
      ...
      interaction: {
        mode: 'ask_user',
        ui_kind: 'confirmation',
        user_message: '신규 환자로 입력되었지만 현재 patient_id로 기존 환자가 검색되었습니다.',
        assistant_question:
          '숫자만 입력해 주세요.\n' +
      ...
      execution_contract: {
        ...
        mode: 'await_user_choice',
        must_show_message: true,
        user_visible_message: '신규 환자로 입력되었지만 현재 patient_id로 기존 환자가 검색되었습니다.',
        must_ask_user: true,
        user_question:
          '1. 기존 환자 신규 방문으로 수정 후 preview 진행\n' +
          '2. patient_id 다시 입력\n' +
  ...
  if (decision === 'reenter_patient_id') {
    return {
      ok: true,
      tool: 'sender_transform',
      request_id: transformResult.request_id,
...
async function runTransformTool(args) {
  ...
  if (hasExistingPatientNewVisitPatientHeaderTouch(args.payload)) {
...
async function runSendTool(args) {
  ...
  if (isNewPatientNewVisitPayload(args.payload)) {
    ...
    if (
      ...
      return {
        ...
        message:
          '현재 환자 존재 여부를 확인할 수 없어 자동 진행을 멈췄습니다.',
        write_allowed: false,
    ...
    if (conflictEnvelope?.stage === 'NEW_PATIENT_PATIENT_FOUND_STAGE1_PREVIEW') {
      return {
        ok: false,
        tool: 'sender_send',
        request_id: transformResult.request_id,
        ...
        message: '먼저 sender_transform에서 충돌 확인 preview를 보여주고 선택을 받은 뒤 다시 진행해야 합니다.',
        write_allowed: false,
        resend_allowed: false,
        ...
        interaction: {
          mode: 'inform',
          ui_kind: 'info',
          user_message: '먼저 충돌 확인 preview에서 선택을 완료한 뒤 전송해 주세요.',
          assistant_question: '',
          required_user_input: null,
          do_not_ask: []
        },
        ...
        execution_contract: {
          ...
          mode: 'stop',
          must_show_message: true,
          user_visible_message: '먼저 충돌 확인 preview에서 선택을 완료한 뒤 전송해 주세요.',
          must_ask_user: false,
          user_question: '',
          accepted_input_type: null,
          allowed_actions: ['show_stop_message'],
    ...
    if (conflictEnvelope?.stage === 'NEW_PATIENT_PATIENT_FOUND_REINPUT_REQUIRED') {
      return {
        ...
        message: '먼저 patient_id를 다시 입력해 preview를 연 뒤 최종 전송해 주세요.',
        write_allowed: false,
        resend_allowed: true,
        ...
        must_open_preview_immediately: true,
        must_not_call_sender_send_after_reinput: true,
        execution_contract: conflictEnvelope.execution_contract || {
          contract_version: '1.0',
          mode: 'await_user_input',
          must_show_message: true,
          user_visible_message:
    ...
    if (conflictDecision === 'switch_existing_new_visit') {
      effectiveInputPayload =
        convertNewPatientConflictToExistingPatientNewVisitPayload(args.payload);
  ...
  if (
    ...
    const headerDecision = parseExistingPatientNewVisitHeaderDecision(args.phase1_decision);
    const finalConfirmation = parseNewVisitFinalConfirmation(args.phase1_decision);
    ...
    if (envelope?.stage === 'EXISTING_PATIENT_NEW_VISIT_STAGE1_PREVIEW') {
      return {
        ...
        message: '먼저 sender_transform에서 patient header 1차 판단 preview를 보여주고 선택을 받은 뒤 다시 진행해야 합니다.',
        write_allowed: false,
        resend_allowed: false,
        ...
        interaction: {
          mode: 'inform',
          ui_kind: 'info',
          user_message: '먼저 preview 1단계에서 patient header 수정 여부를 선택해 주세요.',
          assistant_question: '',
          required_user_input: null,
          do_not_ask: []
        },
        ...
        execution_contract: {
          ...
          mode: 'stop',
          must_show_message: true,
          user_visible_message: '먼저 preview 1단계에서 patient header 수정 여부를 선택해 주세요.',
          must_ask_user: false,
          user_question: '',
          accepted_input_type: null,
          allowed_actions: ['show_stop_message'],
    ...
    if (finalConfirmation !== 'send_now') {
      return {
        ...
        message: 'sender_send는 최종 new visit preview 확인 완료 후에만 실행됩니다. 먼저 sender_transform preview를 보고, phase1_decision.new_visit_full_preview_confirmation 값을 포함해 다시 호출해 주세요.',
        write_allowed: false,
        resend_allowed: false,
        ...
        interaction: {
          mode: 'inform',
          ui_kind: 'info',
          user_message: '먼저 최종 preview를 확인하고 숫자 선택을 완료한 뒤 전송해 주세요.',
          assistant_question: '',
          required_user_input: null,
          do_not_ask: []
        },
        ...
        execution_contract: {
          ...
          mode: 'stop',
          must_show_message: true,
          user_visible_message: '먼저 최종 preview를 확인하고 숫자 선택을 완료한 뒤 전송해 주세요.',
          must_ask_user: false,
          user_question: '',
          accepted_input_type: null,
          allowed_actions: ['show_stop_message'],
  ...
  if (isPhase1ApplicableExistingVisitUpdate(args.payload)) {
    ...
    if (isPatientRecheckResumeFlow && finalConfirmation !== 'send_now') {
      return {
        ...envelope,
        tool: 'sender_send',
        message: 'recheck 후 수정된 patient_id 기준 preview를 다시 열었습니다. 이 preview를 확인한 뒤 최종 전송해 주세요.',
        interaction: {
          ...(envelope.interaction || {}),
          user_message: 'recheck 후 수정된 patient_id 기준 preview입니다. 아래 내용을 확인한 뒤 숫자로 선택해 주세요.'
        },
        execution_contract: {
          ...(envelope.execution_contract || {}),
          mode: 'await_user_choice',
          must_show_message: true,
          user_visible_message: 'recheck 후 수정된 patient_id 기준 preview입니다. 아래 내용을 확인한 뒤 숫자로 선택해 주세요.',
          must_ask_user: true,
          stop_after_response: false
        },
        debug: {
