...
function findPreOpChildRecordIdFromVisitChildren(childrenBody, toothNumber, recordName) {
  ...
  if (exact?.id) {
    return {
      ...
      matched_record_name: safeString(exact?.fields?.['Record name'])
    };
  }
  ...
  if (fallback?.id) {
    return {
      ...
      matched_record_name: safeString(fallback?.fields?.['Record name'])
    };
  }
...
async function buildExistingVisitUpdateCurrentState(transformedPayload) {
  ...
  for (const record of findings) {
    ...
    const childLookup = findPreOpChildRecordIdFromVisitChildren(
      childrenResp.body,
      toothNumber,
      recordName
    );
...
function runHealthTool() {
  return {
    ...
    version: '1.1.8-phase1-existing-visit-update-hotfix8',
