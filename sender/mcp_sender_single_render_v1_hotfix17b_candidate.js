...
function extractNewPatientPatientFoundReplacementPatientId(rawDecision) {
  ...
  const nested = safeString(
    rawDecision.patients?.patient_id ||
...
async function buildNewPatientPatientFoundTransformEnvelope(payload, transformResult, rawDecision) {
  ...
  if (!(patientResp && patientResp.ok)) {
    const stopMessage =
      decision === 'reenter_patient_id'
        ? `새 patient_id ${patientId || '(blank)'} 로 다시 확인하려 했지만 현재 환자 존재 여부를 확인할 수 없어 자동 진행을 멈췄습니다. 잠시 후 다시 시도해 주세요.`
    ...
    return {
      ...
      error: {
        ...
        message:
          decision === 'reenter_patient_id'
            ? '재입력한 patient_id로 신규 환자 claim 검증을 위한 current-state patient lookup이 불가능하여 자동 진행을 차단합니다.'
            : '신규 환자 claim 검증을 위한 current-state patient lookup이 불가능하여 전송을 차단합니다.'
      },
      interaction: {
        mode: 'inform',
        ui_kind: 'info',
    ...
    if (conflictEnvelope?.stage === 'NEW_PATIENT_PATIENT_FOUND_REINPUT_REQUIRED') {
      return {
        ...
        execution_contract: {
          ...
          must_show_message: true,
          user_visible_message:
            '수정된 6자리 patient_id를 다시 입력한 뒤 preview를 열어 주세요.',
          must_ask_user: true,
          user_question: '수정된 6자리 patient_id만 다시 입력해 주세요.',
          accepted_input_type: 'patient_id',
          accepted_format: '6_digit_string',
